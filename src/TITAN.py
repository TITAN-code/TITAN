...
def process_quantification_request(filename):
    ...
    if FILE == "TXT":
        ...
        NAME_CHARGE_DISTRIBUTION,FORCE,N_TERMINAL,C_TERMINAL = input.read_input_pdb(filename)
        if FORCE == "AMBER":
            pdb_amber.charge_distribution_generation_amber(NAME,NAME_CHARGE_DISTRIBUTION,N_TERMINAL,C_TERMINAL)
        elif FORCE == "CHARMM":
            ASPP,GLUP,DISU = input.read_input_charmm(filename)
            pdb_charmm.charge_distribution_generation_charmm(NAME,NAME_CHARGE_DISTRIBUTION,N_TERMINAL,C_TERMINAL,ASPP,GLUP,DISU)
...
def quant(UNIT, NAME, CHARGE_SELECT, CHARGE_SEQ, V1X, V1Y, V1Z, V2X, V2Y, V2Z, XP, YP, ZP):
    ...
    quantification.write_output_quant(UNIT,NAME,NO_CHA,EFX,EFY,EFZ,OEF,EFTOT,XP,YP,ZP,V1X,V1Y,V1Z,V2X,V2Y,V2Z,VX,VY,VZ,COUNT)
...
if __name__ == '__main__':
    ...
    if len(sys.argv[1:]) == 1:
        ...
        print 'Usage:'
        print 'python EFG.py filename.inp'
