...
# NON-STANDARD RESIDUE CAN BE ADDED INTO "top_protein.rtf" WITH THE CHARGE SECTION FROM CHARMM-TYPE TOPOLOGICAL FILE.
# FOR EXAMPLE: 
...
#
result_n = []
with open("N_terminal.rtf","r") as file_n:
   for line in file_n:
...
file_n.close()
#
NO_N = linecount1("N_terminal.rtf")
list_n = range(NO_N)
#
T1_N = range(NO_N)
T2_N = range(NO_N)
...
#
result_c = []
with open("C_terminal.rtf","r") as file_c:
   for line in file_c:
...
file_c.close()
#
NO_C = linecount1("C_terminal.rtf")
list_c = range(NO_C)
#
T1_C = range(NO_C)
T2_C = range(NO_C)
